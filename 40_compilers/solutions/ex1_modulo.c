// SOLUTION 40.1 — Add the modulo operator `%` end to end
// The three TODOs are resolved (marked  <-- TODO).
...
typedef enum {
    ...
    T_PERCENT,                              // <-- TODO 1: new token for '%'
...
static Token lex_next(Lexer *lx) {
    ...
    switch (c) {
        ...
        case '%': tk.type = T_PERCENT; return tk;   // <-- TODO 2: lex '%'
...
static Node *p_term(Parser *p) {
    Node *l = p_factor(p);
    // <-- TODO 3: '%' parses at the same level as '*' and '/'
    while (p->cur.type == T_STAR || p->cur.type == T_SLASH || p->cur.type == T_PERCENT) {
        char op = (p->cur.type == T_STAR) ? '*' : (p->cur.type == T_SLASH) ? '/' : '%';
