// SOLUTION 40.3 — A NEG opcode in the stack VM
// All four TODOs resolved (marked  <-- TODO).
...
typedef enum {
    ...
    OP_NEG                                  // <-- TODO 1: new unary opcode
} OpCode;
...
static long vm_run(const Chunk *c) {
    ...
    for (int ip = 0; ip < c->count; ip++) {
        ...
        switch (in.op) {
            ...
            case OP_NEG: { long a = st[--sp]; st[sp++] = -a; break; }   // <-- TODO 4
        }
    }
    return st[0];
}
