// Exercise 35.1 — Add coalescing to a free-list allocator
//
// The allocator below works, but its my_free only flips the `free` flag -- it never
// MERGES adjacent free blocks. Run it as-is and you'll see free(b) then free(a)
// leave two separate small holes; a later my_malloc for a big block then FAILS even
// though the free bytes would be enough if they were contiguous.
//
// TODO: make my_free coalesce. After flipping b->free = 1, merge b with the
// FOLLOWING block while that neighbour is also free (use next_block + b->size).
// Verify with `make ex1`: after freeing all blocks the dump should show ONE big
// free block, and the final my_malloc(300) should succeed. Solution in
// ../solutions/ex1_coalesce.c; README §6.
...
void my_free(void *ptr) {
    ...
    b->free = 1;
    // TODO: coalesce. While the NEXT block exists and is free, absorb it:
    //   b->size += HDR_SIZE + next->size;  then recompute next from b.
}
