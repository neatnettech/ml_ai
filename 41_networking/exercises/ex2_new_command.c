// Exercise 41.2 — Extend the protocol with a new command (server-side)
//
// The client below sends "SUB 50 8" and expects "RESULT 42". The server's request
// dispatcher already handles PING and ADD; your job is to add a SUB handler that
// subtracts b from a and replies "RESULT <a-b>\n".
//
// Fill in the TODO in `handle_request()`. Then `make ex2` should match `make sol2`.
// Solution in ../solutions/ex2_new_command.c.
...
// ===================== YOUR CODE: extend the dispatcher =====================
static void handle_request(const char *line, char *out, size_t outsz) {
    ...
    // TODO: add an `else if` branch for the new "SUB a b" command.
    //       It needs parts >= 3 and verb == "SUB"; reply "RESULT <a-b>\n"
    //       using strtol(a,NULL,10) - strtol(b,NULL,10), like the ADD branch above.
    else {
